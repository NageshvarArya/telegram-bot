...
def download_instagram_video(instagram_url):
    try:
        ...
        shortcode_match = re.search(r'/(p|reel)/([\w-]+)', instagram_url)
        ...
        logger.warning("The extracted post is not a video.")
        return None
    except instaloader.exceptions.LoginRequiredException:
        logger.error("Instagram requires login. Instaloader cannot fetch this video.")
...
async def process_request(update: Update, context: CallbackContext):
    ...
    if match:
        ...
        if video_url:
            ...
            await update.message.reply_text("⚠️ This is not a valid Instagram video or reel.")
    else:
